...
public class ImageUpload {
	...
	public String imageUpload(
		...
		OutputStream out = null; // 본문내용에 추가한 이미지 파일로 생성할 객체
		long limitSize = 1024 * 1024 * 2; // 업로드 파일 최대 크기(2MB)
		
		// CKEditor 본문 내용에 이미지를 업로드해보면 'upload'라는 키로 파일 데이터가 전달되는 걸 확인할 수 잇다
		...
		if(file != null && file.getSize() > 0 && StringUtils.isNotBlank(file.getName())) {
			...
			if(file.getContentType().toLowerCase().startsWith("image/")) {
				// 업로드한 파일 크기 체크
				if(file.getSize() > limitSize) { // 최대 크기 2MB 보다 클 때
					/*
					 * {
					 * 		"uploaded" : 0,
					 *		"error" : [
					...
					try {
						...
						byte[] bytes = file.getBytes(); // 파일 데이터 얻어보기
						 ...
						 * {
						 * 		"uploaded" : 1,
						 * 		"fileName" : "xxxxxxx-xxxxxx.jpg",
						 * 		"url" : "/resources/xxxxxx-xxxxxx.jpg"
						 * 	}
						 */

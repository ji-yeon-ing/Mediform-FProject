			...
			<div class="modal-content position-relative">
				...
				<div class="modal-body p-0">
					<div class="rounded-top-2 py-2 ps-3 pe-6"
						style="background-color: midnightblue;">
						<h4 class="mb-0 text-white" id="modalExampleDemoLabel">진단서</h4>
					</div>
...
var diagnosisModal = $("#diagnosisModal");
		...
		$.ajax({
			...
			success : function(res){
				if(res.length == 0){ // 검사 결과가 없을때
					...
					// 환자번호
					var patntNoFormat = (res.patntNo).substr(1);
					$("#cdPatntNoDt").text(patntNoFormat);
					...
					if(res.hcrtfDt == null || res.hcrtfDt == "" || res.hcrtfDt == undefined){
						var today = new Date();
						var todayFormat2 = formatDate(today);
				   ...
				   if(res.patntTelno == null || res.patntTelno == "" || res.patntTelno == undefined){
					   ...
					   var telFormat = formatPhoneNumber(res.patntTelno);
				   ...
				   if(res.atfssDt == null || res.atfssDt == "" || res.atfssDt == undefined){
					   ...
					   var atfssDtFormat = formatDate(res.atfssDt);
				   ...
				   if(res.clnicTime == null || res.clnicTime == "" || res.clnicTime == undefined){
					   ...
					   clnicTimeFormat = formatDate(res.clnicTime);
				   ...
				   if(res.hsptlzDe == null || res.hsptlzDe == "" || res.hsptlzDe == undefined){
					   ...
					   var hsptlzDeFormat = formatDate(res.hsptlzDe);
				   ...
				   if(res.realLhstl == null || res.realLhstl == "" || res.realLhstl == undefined){
					   ...
					   var realLhstlFormat = formatDate(res.realLhstl);
					...
					if(res.hcrtfDocNote == null || res.hcrtfDocNote == "" || res.hcrtfDocNote == undefined){
						...
						$("#cdDocNoteDtSpan").text(res.hcrtfDocNote);
  						...
  						// 비고
  						if(hcrtfRm == null || hcrtfRm == "" || hcrtfRm == undefined){
					...
					$(res.selectMedicalTreatmentRecordVOList).each(function(){
						...
						if(this.mdlrtNm != null && this.mdlrtNm != "" ){
							...
							if(this.mdlrtDt == null || this.mdlrtDt == ""){
								...
								formatDtMtr = formatDate(this.mdlrtDt);
							}
							mtrHtml += "<tr>";
							mtrHtml += "<td style='width:135px; height: 0px; text-align: center;'>"+ formatDtMtr + "</td>"
							mtrHtml += "<td colspan='2' style='width:135px; height: 0px;'> # "+ this.mdlrtNm + "</td>"
							mtrHtml += "</tr>";
						}
					});
					...
					$(res.medicalExaminationOrderVOList).each(function(){
						...
						meoHtml += "<tr>";
						meoHtml += "<td style='width:135px; height: 0px; text-align: center;'>"+ formatDtMeo + "</td>"
						meoHtml += "<td style='width:135px; height: 0px;'> # "+ this.hlthchkpNm + "</td>"
	...
	// 등록 버튼 클릭시
 	$("#certificatesInsertBtn").on("click", function(){
 		var noFormat = $("#cdPatntNoDt").text();
 		
 		var patntNo = "P"+noFormat;
		console.log("patntNo : ", patntNo);
